...
namespace Root.Scripts.Utilities.Curves
{
    ...
    public abstract class BaseCurve
    {
        ...
        public (Vector3[] tangents, Vector3[] normals, Vector3[] binormals) ComputeFrenetFrames(int segments,
            bool closed)
        {
            ...
            for (int i = 1; i <= segments; i++)
            {
                ...
                {
                    vec.Normalize();
                    var theta = Mathf.Acos(Mathf.Clamp(Vector3.Dot(tangents[i - 1], tangents[i]), -1,
