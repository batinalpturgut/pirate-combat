...
namespace Root.Scripts.Utilities.Curves
{
    ...
    public class SplineCurve : BaseCurve
    {
        ...
        private bool closed;
        private ESplineType curveType;
        private float tension;
        
        public Vector3[] Waypoints => points;
        public bool Closed => closed;
        public ESplineType CurveType => curveType;
        public float Tension => tension;

        public SplineCurve(Vector3[] points, bool closed = false, ESplineType curveType = ESplineType.Centripetal,
            float tension = 0.5f)
        {
            this.points = points;
            this.closed = closed;
        ...
        public Quaternion GetRotation(float t)
        {
            ...
            Vector3 nextPoint = GetPoint(nextT);

            Vector3 forward = (nextPoint - currentPoint).normalized;
            
            if (forward.sqrMagnitude < 0.000001f && !closed)
            {
                return Quaternion.LookRotation(points[^1] - points[^2], Vector3.up);
            }
        ...
        public override Vector3 GetPoint(float t)
        {
            ...
            if (this.curveType == ESplineType.Centripetal || this.curveType == ESplineType.Chordal)
            {
                // init Centripetal / Chordal Catmull-Rom
                var pow = this.curveType == ESplineType.Chordal ? 0.5f : 0.25f;
            ...
            else if (this.curveType == ESplineType.CatmullRom)
            {
    ...
    public class CubicPoly3D
    {
        ...
        public void InitNonuniformCatmullRom(Vector3 x0, Vector3 x1, Vector3 x2, Vector3 x3, float dt0, float dt1,
            float dt2)
        {
            // compute tangents when parameterized in [t1,t2]
            var t1 = (x1 - x0) / dt0 - (x2 - x0) / (dt0 + dt1) + (x2 - x1) / dt1;
